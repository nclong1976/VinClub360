...
function isValidSupabaseUrl(url?: string): boolean {
  ...
  if (/your_project|your-project|example\.com|placeholder|<[^>]+>/i.test(trimmed)) return false;
  try {
    const parsed = new URL(trimmed);
    return Boolean(parsed.hostname && !parsed.hostname.includes("your_project") && !parsed.hostname.includes("example"));
  } catch {
    return false;
  }
}
...
function isValidSupabaseKey(key?: string): boolean {
  ...
  if (/xxxxxxxx|placeholder|<[^>]+>/i.test(trimmed)) return false;
  return true;
}

const supabaseServiceUrl = process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL;
const supabaseServiceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
const isSupabaseConfigured = isValidSupabaseUrl(supabaseServiceUrl) && isValidSupabaseKey(supabaseServiceRoleKey);
...
if (supabaseAdmin) {
  ...
  console.warn(
    "[Supabase] SUPABASE_URL hoặc SUPABASE_SERVICE_ROLE_KEY chưa được cấu hình hoặc đang chứa giá trị mẫu (như your_project.supabase.co). Các tác vụ Supabase nền (đồng bộ CSKH, nạp/rút, cộng lãi hàng ngày) đang tạm tắt an toàn."
  );
}
...
async function runDailyInterestBatch() {
  ...
  try {
    ...
    if (error) {
      console.warn("[DailyInterest] Lỗi gọi credit_daily_interest_batch:", error.message);
      return;
    }
    const rows = data || [];
    if (rows.length > 0) {
...
function subscribeWithAutoReconnect(createChannel: () => any, label: string) {
  ...
  const connect = () => {
    ...
    channel.subscribe((status: string, err?: any) => {
      // CỰC KỲ QUAN TRỌNG: Nếu channel này đã kích hoạt đóng/reconnect (isTornDown = true),
      // BỎ QUA mọi callback sau đó. Khi gọi removeChannel(channel), thư viện Phoenix
      // channel.leave() sẽ kích hoạt sự kiện onClose() ĐỒNG BỘ nếu socket chưa kết nối,
      // dẫn đến việc callback này tự gọi lại liên tục trong cùng call stack, gây
      // "RangeError: Maximum call stack size exceeded" và lỗi "Exception in PromiseRejectCallback".
      ...
      if (status === "CLOSED" || status === "CHANNEL_ERROR" || status === "TIMED_OUT") {
        ...
        // removeChannel() trả về 1 Promise (không phải chạy đồng bộ). Bọc
        // Promise.resolve().catch() để không bao giờ có promise nào bị bỏ rơi ở đây.
        try {
          if (supabaseAdmin) {
...
async function pollAndForwardUnsentCskhMessages() {
  ...
  try {
    ...
    if (error) {
      ...
      cskhPollingErrorCount += 1;
      if (!isNetworkError || cskhPollingErrorCount <= 3 || cskhPollingErrorCount % 20 === 0) {
        console.warn(
          `[Telegram] Polling CSKH (${isNetworkError ? "kết nối mạng" : "lỗi đọc tin nhắn mới"}, lần ${cskhPollingErrorCount}):`,
          error.message
        );
      }
      return;
    }
    cskhPollingErrorCount = 0;
    ...
    for (const row of rows) {
      ...
      await forwardUserMessageToTelegramGroup(row).catch((e) =>
        console.warn("[Telegram] Polling CSKH: lỗi forward tin nhắn:", e?.message || e)
      );
    }
  } catch (e: any) {
    const isNetworkError = e?.message?.includes("fetch failed") || e?.message?.includes("ENOTFOUND");
    cskhPollingErrorCount += 1;
    if (!isNetworkError || cskhPollingErrorCount <= 3 || cskhPollingErrorCount % 20 === 0) {
      console.warn(
        `[Telegram] Polling CSKH: lỗi không mong đợi (${isNetworkError ? "kết nối mạng" : "lỗi"}, lần ${cskhPollingErrorCount}):`,
        e?.message || e
      );
    }
  }
