...
export default function ProjectsTab({ filterRequest }) {
  ...
  const [modalTab, setModalTab] = useState("info"); // "info" | "notify"
        ...
        filtered.map((p) => {
          ...
          return (
            ...
            >
              ...
              {/* Action Toolbar: Edit, Notify, Delete & Investment Toggle Switch */}
              <div className="flex items-center justify-between gap-2 mt-2.5 pt-2 border-t border-gray-100 flex-wrap">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 text-[10.5px] font-bold flex items-center gap-1 transition-all"
                  >
                  ...
                  <button
                    ...
                    title="Gửi thông báo tới người dùng của dự án này"
                    className="px-2.5 py-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 text-[10.5px] font-bold flex items-center gap-1 transition-all"
                  >
                    <Megaphone className="w-3 h-3 text-blue-600" /> Thông báo
                  </button>
                  <button
...
function ProjectEditModal({ project, initialTab = "info", onClose, onSave }) {
  ...
  // ================= NOTIFICATION TAB STATE & LOGIC =================
  const [notifTarget, setNotifTarget] = useState("investors"); // "investors" | "all"
  const [notifTitle, setNotifTitle] = useState("");
  const [notifContent, setNotifContent] = useState("");
  const [notifImage, setNotifImage] = useState(form.image || "");
  ...
  // Lấy danh sách những người đã đầu tư vào dự án này
  const fetchProjectInvestors = async () => {
    if (!project?.id) return;
    setLoadingInvestors(true);
    try {
      const txList = await base44.entities.Transaction.list("-created_date", 500);
      const projIdStr = String(project.id);
      const projTitleLower = (project.title || form.title || "").trim().toLowerCase();
      const projNameLower = (project.name || form.name || "").trim().toLowerCase();

      const matched = (txList || []).filter((tx) => {
        if (!tx.user_id) return false;
        if (tx.project_id && String(tx.project_id) === projIdStr) return true;
        if (tx.project_title && tx.project_title.trim().toLowerCase() === projTitleLower) return true;
        if (tx.project_name && tx.project_name.trim().toLowerCase() === projNameLower) return true;
        return false;
      });
      ...
      matched.forEach((tx) => {
        ...
        prev.totalAmount += Number(tx.amount) || 0;
        prev.txCount += 1;
        if (tx.user_email && !prev.user_email) prev.user_email = tx.user_email;
        if (tx.user_name && !prev.user_name) prev.user_name = tx.user_name;
        invMap.set(uid, prev);
      });

      const invArray = Array.from(invMap.values()).sort((a, b) => b.totalAmount - a.totalAmount);
      setInvestors(invArray);
      if (invArray.length === 0) {
        setNotifTarget("all");
      }
    } catch (e) {
      console.error("Error fetching project investors:", e);
    } finally {
  ...
  useEffect(() => {
    if (project?.id) {
      ...
      // Khởi tạo sẵn bản thảo từ thông tin dự án
      const draft = buildProjectAnnouncementDraft({ ...form, id: project.id });
      setNotifTitle(draft.title);
      setNotifContent(draft.content);
      setNotifImage(form.image || "");
    }
  }, [project?.id]);
  ...
  const handleApplyPreset = (type) => {
    ...
    if (type === "open") {
      const draft = buildProjectAnnouncementDraft({
        ...form,
        id: project?.id,
      });
      setNotifTitle(draft.title);
      setNotifContent(draft.content);
      setNotifImage(form.image || "");
      toast.info("Đã áp dụng mẫu mở bán / ra mắt dự án");
    } else if (type === "progress") {
      setNotifTitle(`📈 Cập nhật tiến độ dự án: ${form.title}`);
      setNotifContent(
        `Kính gửi Quý nhà đầu tư dự án ${form.title},\n\nTiến độ triển khai dự án hiện đạt ${form.progress}% với mức ${
          isDailyAccrualCategory(form.category) ? "lãi suất" : "lãi suất toàn kỳ"
        } ${form.total_term_interest_rate}%. Mọi hạng mục đang được giám sát chặt chẽ đảm bảo quyền lợi tốt nhất cho Quý hội viên.\n\nTrân trọng cảm ơn Quý khách!`
      );
      setNotifImage(form.image || "");
      toast.info("Đã áp dụng mẫu cập nhật tiến độ");
    } else if (type === "notice") {
      setNotifTitle(`🔔 Thông báo quan trọng về dự án: ${form.title}`);
      setNotifContent(
        `Kính gửi Quý khách hàng tham gia dự án ${form.title},\n\nBan Quản trị VinClub xin gửi tới Quý khách thông tin cập nhật mới nhất về các mốc thanh toán và chính sách bảo lãnh hợp đồng. Quý khách vui lòng kiểm tra chi tiết hợp đồng và liên hệ CSKH khi cần hỗ trợ.`
      );
      setNotifImage(form.image || "");
      toast.info("Đã áp dụng mẫu thông báo quan trọng");
    }
  };
  ...
  const handleSendProjectNotification = async () => {
    ...
    if (notifTarget === "investors" && investors.length === 0) {
      toast.error("Dự án chưa có nhà đầu tư nào. Vui lòng chọn 'Tất cả hội viên' để quảng bá.");
      return;
    }
    ...
    setSentSuccessInfo(null);
    try {
      let finalImg = notifImage.trim();
      if (imageFile) {
        try {
          const res = await base44.integrations.Core.UploadFile({ file: imageFile });
          if (res?.file_url) finalImg = res.file_url;
          else if (imagePreview) finalImg = imagePreview;
        } catch {
          if (imagePreview) finalImg = imagePreview;
        }
      }

      const { openIso, closeIso } = resolveProjectOpenClose(form);
      const projectSnapshot = {
        project_id: project?.id || undefined,
        project_title: form.title,
        project_category: form.category,
        project_rate: form.total_term_interest_rate,
        project_duration_minutes: form.term_duration_minutes,
        project_min_amount: form.minAmount,
        project_scale: form.scale,
        project_open_at: openIso || undefined,
        project_close_at: closeIso || undefined,
      };

      if (notifTarget === "investors") {
        for (const inv of investors) {
          await base44.entities.Notification.create({
            ...
            type: "project",
            user_id: inv.user_id,
            user_email: inv.user_email || undefined,
            user_name: inv.user_name || undefined,
            is_read: false,
            created_date: new Date().toISOString(),
            ...projectSnapshot,
            extra: projectSnapshot,
          });

          // Đồng thời gửi 1 tin nhắn vào kênh chat CSKH của hội viên đó
          try {
            await base44.entities.Message.create({
              sender: "admin",
              conversation_id: inv.user_id,
              user_id: inv.user_id,
              content: `📢 [Thông báo dự án: ${form.title}]\n\n${notifTitle.trim()}\n\n${notifContent.trim()}`,
              created_date: new Date().toISOString(),
            });
          } catch {}
        }
        ...
        toast.success(`✅ Đã gửi thông báo thành công tới ${investors.length} nhà đầu tư của dự án!`);
      } else {
        await base44.entities.Notification.create({
          title: notifTitle.trim(),
          content: notifContent.trim(),
        ...
        toast.success(`✅ Đã phát thông báo dự án "${form.title}" tới toàn bộ hội viên!`);
      }

      // Kích hoạt đồng bộ chuông thông báo tức thì
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-xs p-2 sm:p-4" onClick={onClose}>
      <div className="w-full max-w-lg sm:max-w-xl bg-white rounded-2xl max-h-[88vh] flex flex-col shadow-2xl border border-gray-200 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        {/* Sticky Header */}
        <div className="sticky top-0 z-10 bg-white border-b border-gray-100 shrink-0">
          <div className="flex items-center justify-between px-4 py-3">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="w-8 h-8 rounded-xl bg-[#948154]/10 text-[#948154] flex items-center justify-center shrink-0">
                <Building2 className="w-4 h-4" />
              </div>
              <div className="min-w-0">
                <h2 className="text-[13px] font-bold text-black truncate leading-snug">
                  {project?.id ? form.title || "Chỉnh sửa dự án" : "Thêm dự án mới"}
                </h2>
                <div className="flex items-center gap-1.5 text-[10px] text-gray-400">
                  <span className="font-semibold text-gray-600">{form.category}</span>
                  {project?.id && (
                    <>
                      <span>•</span>
                      <span>Mã ID: {project.id}</span>
                    </>
                  )}
                </div>
              </div>
            </div>
            <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-500 transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>

          {/* Sub-tabs switch (Chỉnh sửa thông tin / Gửi thông báo) */}
          {project?.id && (
            <div className="flex border-t border-gray-100 px-4 bg-gray-50/70 gap-2">
              <button
                type="button"
                onClick={() => setActiveTab("info")}
                className={`py-2 px-3 text-[11.5px] font-bold border-b-2 flex items-center gap-1.5 transition-all cursor-pointer ${
                  activeTab === "info"
                    ? "border-[#948154] text-[#948154]"
              ...
              >
                ...
                {investors.length > 0 && (
                  <span className="px-1.5 py-0.2 rounded-full text-[9px] font-black bg-blue-100 text-blue-700">
                    {investors.length}
                  </span>
                )}
              </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {activeTab === "notify" && project?.id ? (
            /* ================= GỬI THÔNG BÁO DỰ ÁN ================= */
            <div className="space-y-4">
              {/* Project Overview Card */}
              <div className="flex items-center gap-3 p-3 rounded-xl border border-blue-100 bg-blue-50/30">
                {form.image ? (
                  <img src={form.image} alt="" className="w-12 h-12 rounded-xl object-cover shrink-0 border border-gray-200" />
                ) : (
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-amber-100 text-amber-900">
                      Lãi suất: {form.total_term_interest_rate}%
                    </span>
                  </div>
                  <h4 className="text-[12.5px] font-bold text-gray-900 truncate mt-0.5">{form.title}</h4>
                  <p className="text-[10px] text-gray-500">
                    Kỳ hạn: {termValueInUnit} {termUnit} • Tối thiểu: {Number(form.minAmount || 0).toLocaleString("vi-VN")} ₫
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-gray-700 flex items-center justify-between">
                  ...
                  {loadingInvestors && <span className="text-[9.5px] text-gray-400 font-normal">Đang kiểm tra nhà đầu tư...</span>}
                </label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => setNotifTarget("investors")}
                    className={`p-3 rounded-xl border text-left transition-all cursor-pointer ${
                      notifTarget === "investors"
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <Users className={`w-4 h-4 ${notifTarget === "investors" ? "text-blue-600" : "text-gray-500"}`} />
                        <span className="text-[11.5px] font-bold text-gray-900">Nhà đầu tư dự án này</span>
                      </div>
                      <span className={`text-[10px] font-black px-1.5 py-0.2 rounded-full ${investors.length > 0 ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-500"}`}>
                        {investors.length}
                      </span>
                    </div>
                    <p className="text-[9.5px] text-gray-500 mt-1 leading-normal">
                      Gửi thông báo & tin nhắn CSKH trực tiếp tới các hội viên đang nắm giữ hợp đồng dự án này.
                    </p>
                  </button>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <Megaphone className={`w-4 h-4 ${notifTarget === "all" ? "text-amber-600" : "text-gray-500"}`} />
                        <span className="text-[11.5px] font-bold text-gray-900">Toàn bộ hội viên</span>
                      </div>
                      <span className="text-[10px] font-black px-1.5 py-0.2 rounded-full bg-amber-100 text-amber-800">
                        Broadcast
                      </span>
                    </div>
                    <p className="text-[9.5px] text-gray-500 mt-1 leading-normal">
                      Phát thông báo quảng bá ra mắt / mở nhận vốn dự án này đến toàn bộ người dùng ứng dụng.
                    </p>
                  </button>
                </div>
              </div>

              {/* List of matched investors (if selected) */}
              {notifTarget === "investors" && (
                <div className="rounded-xl border border-gray-200 bg-gray-50/50 p-2.5">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[10.5px] font-bold text-gray-700 flex items-center gap-1">
                      ...
                      Danh sách nhà đầu tư ({investors.length})
                    </span>
                    <button
                      type="button"
                      onClick={fetchProjectInvestors}
                      className="text-[9.5px] text-blue-600 hover:underline flex items-center gap-1"
                    >
                      Làm mới
                    </button>
                  </div>
                  ...
                  {investors.length > 0 ? (
                    <div className="max-h-28 overflow-y-auto divide-y divide-gray-100 rounded-lg bg-white border border-gray-200">
                      {investors.map((inv) => (
                        <div key={inv.user_id} className="p-2 flex items-center justify-between text-[10.5px]">
                          <div className="min-w-0 pr-2">
                            <p className="font-bold text-gray-900 truncate">{inv.user_name || inv.user_email || `Hội viên #${inv.user_id.slice(-6)}`}</p>
                            <p className="text-[9px] text-gray-400 truncate">{inv.user_email || inv.user_id}</p>
                          </div>
                          <div className="text-right shrink-0">
                            <span className="font-bold text-emerald-600">{Number(inv.totalAmount || 0).toLocaleString("vi-VN")} ₫</span>
                            <p className="text-[8.5px] text-gray-400">{inv.txCount} giao dịch</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-center py-3 px-2 text-[10px] text-gray-500 bg-white rounded-lg border border-dashed border-gray-200">
                      Chưa có hội viên nào đầu tư dự án này. Bạn có thể chọn mục <b>"Toàn bộ hội viên"</b> ở trên để phát thông báo quảng bá.
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="space-y-1.5">
                <label className="text-[10.5px] font-bold text-gray-700 flex items-center gap-1">
                  <Sparkles className="w-3.5 h-3.5 text-amber-600" /> Mẫu soạn thảo nhanh:
                </label>
                <div className="flex gap-1.5 flex-wrap">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-blue-50 hover:bg-blue-100 text-blue-800 border border-blue-200 transition-colors"
                  >
                    📈 Mẫu Cập nhật tiến độ
                  </button>
                  <button
                    type="button"
                    onClick={() => handleApplyPreset("notice")}
                    className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-purple-50 hover:bg-purple-100 text-purple-800 border border-purple-200 transition-colors"
                  >
                    🔔 Mẫu Lưu ý quan trọng
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-[10px] font-bold text-gray-700 block mb-1">Tiêu đề thông báo (*):</label>
                  <input
                    value={notifTitle}
                    onChange={(e) => setNotifTitle(e.target.value)}
                    placeholder="VD: Cập nhật tiến độ và chi trả lợi nhuận dự án..."
                    className="w-full px-2.5 py-1.5 rounded-lg border border-gray-200 text-[11px] focus:outline-none focus:border-blue-600 font-medium"
                  />
                </div>

                <div>
                  <label className="text-[10px] font-bold text-gray-700 block mb-1">Nội dung chi tiết thông báo (*):</label>
                  <textarea
                    value={notifContent}
                    onChange={(e) => setNotifContent(e.target.value)}
                    rows={4}
                    placeholder="Nhập nội dung thông báo gửi đến Quý khách hàng..."
                    className="w-full px-2.5 py-1.5 rounded-lg border border-gray-200 text-[11px] focus:outline-none focus:border-blue-600 resize-none font-normal leading-relaxed"
                  />
                </div>

                {/* Notification Image */}
                <div>
                  <label className="text-[10px] font-bold text-gray-700 block mb-1">Ảnh đính kèm thông báo (tùy chọn):</label>
                  <div className="flex gap-2 items-center">
                    <input
                      value={notifImage}
                      onChange={(e) => {
                        setNotifImage(e.target.value);
                        setImagePreview(e.target.value);
                        setImageFile(null);
                      }}
                      placeholder="https://... hoặc tải ảnh lên"
                      className="flex-1 px-2.5 py-1.5 rounded-lg border border-gray-200 text-[11px] focus:outline-none focus:border-blue-600"
                    />
                    <input
                      type="file"
                      ref={fileInputRef}
                    ...
                    <button
                      ...
                      className="px-2.5 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 text-[10.5px] font-bold flex items-center gap-1 shrink-0"
                    >
                    ...
                    {form.image && (
                      <button
                        ...
                        title="Dùng ảnh hiện tại của dự án"
                        className="px-2 py-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 text-[10px] font-bold shrink-0"
                      >
                        Ảnh dự án
                      </button>
                    )}
                  </div>
                  {(imagePreview || notifImage) && (
                    <div className="mt-2 relative w-24 h-16 rounded-lg overflow-hidden border border-gray-200">
                      <img src={imagePreview || notifImage} alt="" className="w-full h-full object-cover" />
                      <button
                        type="button"
                        onClick={() => {
                          setImagePreview("");
                        ...
                        className="absolute top-1 right-1 w-4 h-4 rounded-full bg-black/60 text-white flex items-center justify-center text-[9px]"
                      >
                        ✕
                      </button>
                    </div>
              ...
              {/* Project Snapshot Link Notice */}
              <div className="p-2.5 rounded-xl bg-blue-50/70 border border-blue-100 text-[10.5px] text-blue-900 flex items-start gap-2">
                <Bell className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
                <div className="leading-relaxed">
                  <span className="font-bold">Đính kèm liên kết dự án:</span> Thông báo này sẽ tự động gắn thẻ snapshot của dự án <b>"{form.title}"</b>. Khi hội viên bấm vào thông báo trong chuông thông báo, hệ thống sẽ mở trực tiếp trang đầu tư và làm nổi bật dự án này.
                </div>
              </div>

              {/* Success Info Banner if recently sent */}
              {sentSuccessInfo && (
                <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-[11px] flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      Đã gửi thành công tới <b>{sentSuccessInfo.count}</b> {sentSuccessInfo.target === "investors" ? "nhà đầu tư" : "hội viên"}!
                    </span>
                  </div>
                  <button
                    type="button"
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-[11.5px] font-bold shadow-md transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed"
              >
                {sendingNotif ? (
                  <>
                  ...
                  <>
                    ...
                    <span>
                      Gửi thông báo tới {notifTarget === "investors" ? `${investors.length} nhà đầu tư của dự án` : "toàn bộ hội viên"}
                    </span>
                  </>
                )}
              </button>
