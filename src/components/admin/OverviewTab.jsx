...
import { Users, FileCheck, FolderOpen, TrendingUp, Key, Copy, Check, ArrowDownToLine, ArrowUpFromLine } from "lucide-react";
...
export default function OverviewTab({ stats = {} }) {
  ...
  const cards = [
    ...
    { label: "Nạp tiền chờ duyệt", value: fmt(pendingDeposits), icon: ArrowDownToLine, color: "bg-emerald-50 text-emerald-600", highlight: pendingDeposits > 0 },
    { label: "Rút tiền chờ duyệt", value: fmt(pendingWithdrawals), icon: ArrowUpFromLine, color: "bg-amber-50 text-amber-600", highlight: pendingWithdrawals > 0 },
    { label: "Hợp đồng chờ duyệt", value: fmt(pendingContracts), icon: FileCheck, color: "bg-orange-50 text-orange-500", highlight: pendingContracts > 0 },
  ];
