...
export default function TransactionsTab({ initialSearchQuery = "", onNavigateToChat = null }) {
  ...
  const handleCreateDemoTx = useCallback(async (type) => {
    try {
      const selectedType = type || (isDeposit ? "deposit" : "withdraw");
      const targetUser = users.find((u) => u.role !== "admin") || users[0] || {
        id: "u_member_demo",
        full_name: "Nguyễn Văn Nam",
        email: "vannam.demo@gmail.com",
      };
      const amount = selectedType === "deposit" ? 10000000 : 5000000;
      const code = (selectedType === "deposit" ? "VCD" : "VCW") + Date.now().toString().slice(-8);
      ...
      const newTx = {
        ...
        description: `Yêu cầu ${selectedType === "deposit" ? "nạp tiền vào ví" : "rút tiền về ngân hàng"} (${code})`,
        code,
        bank_name: selectedType === "withdraw" ? "Techcombank" : undefined,
        account_number: selectedType === "withdraw" ? "190388889999" : undefined,
        account_holder: selectedType === "withdraw" ? targetUser.full_name || "NGUYEN VAN NAM" : undefined,
        created_date: new Date().toISOString(),
      };

      await base44.entities.WalletTransaction.create(newTx);
      toast.success(`✅ Đã tạo lệnh ${selectedType === "deposit" ? "nạp" : "rút"} ${fmt(amount)} VNĐ (${code})`);
      setFilterStatus("pending");
      fetchTxs(true);
    } catch (e) {
      toast.error("Không thể tạo lệnh thử nghiệm");
    }
  }, [isDeposit, users, fetchTxs]);
  ...
  return (
    <div className="space-y-4">
      ...
      {filteredList.length === 0 ? (
        <div className="bg-white rounded-2xl p-8 text-center border border-gray-100 shadow-xs space-y-4">
          ...
          <div className="space-y-1 max-w-md mx-auto">
            <p className="text-[14px] font-bold text-gray-800">
              {filterStatus === "pending"
                ? `Hiện không có lệnh ${isDeposit ? "Nạp tiền" : "Rút tiền"} nào đang chờ duyệt`
                : "Không tìm thấy giao dịch nào phù hợp"}
            </p>
            <p className="text-[11px] text-gray-500 leading-relaxed">
              {filterStatus === "pending" && activeTxs.length > 0
                ? `Hệ thống có ${activeTxs.length} giao dịch ${isDeposit ? "nạp" : "rút"} ở các trạng thái khác (Đã duyệt / Từ chối). Bạn có thể bấm xem "Tất cả" hoặc tạo lệnh mẫu thử nghiệm bên dưới.`
                : filterStatus === "pending"
                ? `Tất cả các lệnh ${isDeposit ? "nạp tiền" : "rút tiền"} gửi từ hội viên sẽ xuất hiện tại đây để Quản trị viên duyệt. Bạn cũng có thể bấm nút tạo lệnh mẫu để kiểm tra tính năng.`
                : "Vui lòng thử chọn trạng thái khác hoặc xóa từ khóa tìm kiếm."}
            </p>
          </div>

          <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
            {filterStatus !== "all" && activeTxs.length > 0 && (
              <button
                onClick={() => setFilterStatus("all")}
                className="px-3.5 py-2 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 text-[11px] font-bold transition-all cursor-pointer"
              >
                Xem tất cả giao dịch ({activeTxs.length})
              </button>
            )}
            <button
              onClick={() => handleCreateDemoTx(isDeposit ? "deposit" : "withdraw")}
              className="px-3.5 py-2 rounded-xl bg-[#948154] hover:bg-[#837046] text-white text-[11px] font-bold shadow-xs flex items-center gap-1.5 transition-all cursor-pointer"
            >
              <Sparkles className="w-3.5 h-3.5" /> Tạo lệnh {isDeposit ? "Nạp" : "Rút"} mẫu thử nghiệm
            </button>
          </div>
        </div>
      ) : (
