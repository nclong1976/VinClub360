...
async function fetchFromSupabase(entityName) {
  if (!isSupabaseConfigured) return getLocalStore(entityName);
  try {
...
function getLocalStore(name) {
  try {
    ...
    if (name === 'News') {
      let modified = false;
      const defaultNews = seedData.News || [];
      for (const defN of defaultNews) {
        const idx = items.findIndex((n) => n.id === defN.id || (n.title && n.title.toLowerCase() === defN.title.toLowerCase()));
        if (idx === -1) {
          items.push(defN);
          modified = true;
        } else {
          // Merge sections and image if missing
          const existing = items[idx];
          if ((!existing.sections || existing.sections.length === 0) && defN.sections?.length > 0) {
            existing.sections = defN.sections;
            modified = true;
          }
          if (!existing.readingTime && defN.readingTime) {
            existing.readingTime = defN.readingTime;
            modified = true;
          }
        }
      }
...
class LocalEntityClient {
  ...
  async _sourceItems() {
    if (!isSupabaseConfigured || !SUPABASE_READABLE_ENTITIES.has(this.entityName)) {
  ...
  async create(data) {
    ...
    if (isSupabaseConfigured) {
      try {
        ...
        console.warn("Lỗi đồng bộ Supabase (create):", e);
      }
    }
